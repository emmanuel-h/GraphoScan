...
void GraphoScan::mySelectBg(const char * fileName_video, const char * fileName_pt_bg)
{
	...
	while (true)
	{
		...
		//isFinished est mis à true quand les 4 points ont été selectionnés à la souris
		if (isFinished)
		{
	...
	//ajoute des points pour obtenir une zone carrée (maybe)
...
//prend une video en entrée, applique dessus le tracking KCF
//en sortie : - une video composée de la video originale et du tracker
//			  - les points correspondants au centre du tracker dans les frames successives
//			  - une video composée des 2 videos precedentes
void GraphoScan::myTrackerKCF(const char* filename, bool isTransPerspective)
{
	...
	if ((skipFrame == 'y')|| (skipFrame == 'Y'))
	{
		while (true)
		{
			...
			{
				//set ajoute une propriété à la VideoCapture, ici la prochaine frame a être décodée
	...
	for (;nFrameStart + frameCount < nFrameEnd;)
	{
		...
		- imgSrc = video originale avec le temps passé
		...
		*/
		...
		//déssiner le trajectoire du point que l'on suit
		...
		//enregistrer deux images dans une seule fenêtre
		
		// fenetre de taille de 2 vidéos
		cv::Mat imgAll = cv::Mat(imgRoi.rows, imgRoi.cols * 2, imgRoi.type());
		// image de la roi surimprimée sur l'image de base
		cv::Mat imgTmp = imgRoi.clone() + imgSuivi.clone();
		//copie imgTmp dans la 1ere moitié d'imgAll
...
void GraphoScan::calcImgPtsAndImgTrack()
{
	...
	//les points sont reliés et mis en noirs et blancs
	...
	//parce que le type de cet image n'est pas correspondant à l'enregirement, il faut la transformation.
...
//	cv::Mat translationMatrix = (cv::Mat_<double>(3, 1) << 21.00185, 178.03986, 18.90310);
	...
	//Données matlab fournies en dur 
	//a modifier
...
//a tester : 1er = épaissir le trait 
...
//pts = 4 points du quadrilatère de selection
