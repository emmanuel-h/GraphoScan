...
bool ReadInnerParam(const char* filename, long double cm[3][3], long double
...

{
  ...
  else cout << "Number of cameras detected: " << numCameras << endl;
  ...
  if (error != PGRERROR_OK)

    {
      ...
      std::cout << "Failed to get camera0 info from camera0" << 
  ...
  if (error1 != PGRERROR_OK)

    {
      ...
      std::cout << "Failed to get camera1 info from camera1" << 
  ...
  if (chV == 'y' || chV == 'Y'){
    ...
    outputVideo.open(NAME_R, CV_FOURCC('X', 'V', 'I', 'D'), 30, 

		     cv::Size(1288, 964), true);
    ...
    outputVideo1.open(NAME_L, CV_FOURCC('X', 'V', 'I', 'D'), 30, 

		      cv::Size(1288, 964), true);

    if (!outputVideo1.isOpened()){

      cout << "could not open the output video1 for write" <<
  ...
  while (key != 27)
	...
	(double)rgbImage.GetRows();
      ...
      cv::Mat image = cv::Mat(rgbImage.GetRows(), rgbImage.GetCols(), 

			      CV_8UC3, rgbImage.GetData(), rowBytes);

      //cv::imshow("rightCamera", image);

      unsigned int rowBytes1 = (double)rgbImage1.GetReceivedDataSize() 

	/ (double)rgbImage1.GetRows();

      cv::Mat image1 = cv::Mat(rgbImage1.GetRows(), 

			       rgbImage1.GetCols(), CV_8UC3, rgbImage1.GetData(), rowBytes1);
      ...
      cv::Mat imageROI(sizeL.height, sizeL.width + sizeR.width, 

		       CV_8UC3);

      cv::Mat wright(imageROI, cv::Rect(sizeL.width, 0, sizeR.width, 

					sizeR.height));

      image.copyTo(wright);

      cv::Mat wleft(imageROI, cv::Rect(0, 0, sizeL.width, 
	...
	PicNameR = date + "\\" + date + right + int2str(numCalib) 

	  + format;

	PicNameL = date + "\\" + date + left + int2str(numCalib) 
	...
	PicNameR = date + "\\" + date + right + pic + 

	  int2str(numPic) + format;

	PicNameL = date + "\\" + date + left + pic + 
  ...
  cout << "=====>disCoeffs_L is:" << endl << distCoeffs_L << endl << 

    endl;

  cout << "=====>cameraMatrix_R is: " << endl << cameraMatrix_R << endl;

  cout << "=====>disCoeffs_L is:" << endl << distCoeffs_R << endl << 

    endl;

  cv::Mat map1_L, map1_R, map2_L, map2_R;

  cv::initUndistortRectifyMap(cameraMatrix_L, distCoeffs_L, cv::Mat(), 

			      cv::Mat(), imageSize, CV_32F, map1_L, map2_L);

  cv::initUndistortRectifyMap(cameraMatrix_R, distCoeffs_R, cv::Mat(), 

			      cv::Mat(), imageSize, CV_32F, map1_R, map2_R);
  ...
  undist_L.open("undist_XIAODI_L.avi", CV_FOURCC('X', 'V', 'I', 'D'), 10, 
  ...
  undist_R.open("undist_XIAODI_R.avi", CV_FOURCC('X', 'V', 'I', 'D'), 10, 
  ...
  Four.open("total_image_four_in_one_XIAODI.avi", CV_FOURCC('X', 'V', 

							    'I', 'D'), 10, cv::Size(1288 * 2, 964 * 2), true);
  ...
  while (!stop){
    ...
    if (cv::waitKey(30) == 27 || i > 

	cam_L.get(CV_CAP_PROP_FRAME_COUNT))
